import {
  FeatureDef,
  featureDefs,
  featureDefsMapByCategory,
  getFeatureDef,
} from "@/app/screener/screener-store";
...
export default function FeatureCharts({
  ...
  const variationLabels = featureDef.variations.map((e) => {
    return { value: e.code, label: e.variations.join(", ") };
  });
    ...
    <>
      {featureCodes.map((c) => {
        ...
        const features = featureData.feature
          ? featureData.feature.features.filter((e) =>
              variationCodes.includes(e.fName)
            )
          : [];
        console.log(featureDef);
        console.log(featureData);
        const stockPercentile = featureData.stockPercentile
          ? Object.entries(featureData.stockPercentile.percentiles).filter(
              (e) => variationCodes.includes(e[0])
            )
          : [];

        const sectorPercentile = featureData.sectorPercentile
          ? Object.entries(featureData.sectorPercentile.percentiles).filter(
              (e) => variationCodes.includes(e[0])
            )
          : [];
        ...
        return features.length > 0 ? (
          <FeatureCharts
            key={c}
            symbol={featureData.feature.symbol}
            category={category}
            featureDef={featureDef}
