...
import {
  ...
  tableDataStore,
  fetchScreenerData,
  getFeatureDefByVariationCode,
  getSelectedScreenerParam,
} from "./screener-store";
...
const linkCellRenderer = ({ value }: { value: string }) => (
  <Anchor target="_blank" href={`/analysis/${value}`}>
    {value}
  </Anchor>
);

function getColDefs(
  requestObj: SelectedFeaturesForm,
  compDefMap: Map<string, CompanyDef>
) {
  if (!requestObj) {
    return [];
  }
  ...
  const defaultColumnDefs = [
    ...
    },
    {
      ...
      valueGetter: (params: any) =>
        compDefMap.get(params.data.percentile.symbol)?.name,
      headerTooltip: "Name",
      // minWidth: 200,
  ...
  // @ts-ignore
  const valueColDefs = requestObj.features.flatMap(
    (e: { feature: string; lowerIsBetter: boolean }) => {
      ...
      function getValue(params: any) {
        const value = params.data.percentile.percentiles[e.feature]
          ? params.data.percentile.percentiles[e.feature]["v"]
          : 0;
    ...
    <>
      {
        ...
        <AgGridReact
          className={`${styles.screenerTable} ag-theme-alpine`}
          theme="legacy"
          ref={gridRef}
          rowData={store?.response}
          // @ts-ignore
          columnDefs={
            store?.request ? getColDefs(store.request, compDefMap) : []
